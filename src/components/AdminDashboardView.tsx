...
export const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({
  ...
  const handleLiberar10Creditos = (proId: string) => {
    StorageService.addProfessionalCredits(proId, 10, 9.99, '02598018796');
    onRefresh();
  };
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredPros.map((pro) => (
                  <tr key={pro.id} className="hover:bg-slate-50/60 transition">
                    ...
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center gap-1 text-[11px] text-teal-800 font-bold bg-teal-50 px-2 py-0.5 rounded-md border border-teal-200">
                        <Coins className="w-3 h-3 text-amber-500" />
                        <span>{pro.oportunidadesDisponiveis ?? 10} Créditos</span>
                      </span>
                    </td>
                    <td className="py-3 px-3">
                      <span
                    ...
                    <td className="py-3 px-3 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          onClick={() => handleLiberar10Creditos(pro.id)}
                          className="px-2 py-1 bg-amber-100 hover:bg-amber-200 text-amber-900 rounded-md text-[10px] font-bold cursor-pointer flex items-center gap-1"
                          title="Liberar 10 oportunidades (Recarga PIX R$ 9,99)"
                        >
